...
pub fn can_move_to(
    ...
    for piece in other_pieces {
        match piece {
            PieceType::King(_, other_king_position, _) => {
            ...
            PieceType::Pawn(_, pawn_positionn, _, _) => {
...
pub fn is_check(king: PieceType, board: &Board) -> bool {
    match king {
        PieceType::King(color, position, _) => {
            let other_pieces = match color {
                Color::Black => board.get_all_white_pieces(),
                Color::White => board.get_all_black_pieces(),
            };
                ...
                if let Some(square) = tmp_board.squares.get((current_index + i) as usize) {
                    if let Some(piece) = &square.piece {
                        if piece.color() != color {
                            ...
                            if !is_check(PieceType::King(color, next_position, u8::MAX), &tmp_board)
                            {
                                return false;
                            }
                        }
...
mod test {
    ...
    fn test_king_unsafe_move() {
        ...
        let black_queen = PieceType::Queen(Color::Black, Position::new('f', 7), u8::MAX);
        ...
        let black_pawn = PieceType::Pawn(Color::Black, Position::new('d', 6), 1, false);
        ...
        let black_king = PieceType::King(Color::Black, Position::new('d', 2), u8::MAX);
    ...
    fn test_king_check() {
        init();
        let mut board = Board::empty();
        let king = PieceType::King(Color::Black, Position::new('e', 8), u8::MAX);
